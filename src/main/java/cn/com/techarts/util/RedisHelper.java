...
public final class RedisHelper {
	...
	public static<T extends UniqueObject> Map<Integer, T> getObjects2(int cache, List<String> keys, Class<T> t){
		var result = new HashMap<Integer, T>();
		if(keys == null || keys.isEmpty()) return result;
		try(Jedis connection = getConnection()){
			if(connection == null) return result;
			connection.select(cache);
			...
			if(Empty.is(resultSet)) return result;
			for(var res : resultSet) {
				var item = deserialize(res, t, false);
	...
	public static long queuedMessages(String channel) {
		...
		return RedisHelper.getList(CACHE_MSGQUEUE, channel).size();
	}
	 ...
	 */
	public static void subscribe(String channel, EventSubscriber subscriber) {
		for(;;) {
	...
	public static String serialize(Object object) {
		...
		var result = Codec.encode(object, true);
		return result;
	}
